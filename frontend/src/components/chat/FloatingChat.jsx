...
export default function FloatingChat() {
  ...
  return (
    <>
      {open && (
        <div className="fixed bottom-20 right-5 w-80 h-[460px] bg-white rounded-2xl shadow-2xl border border-slate-200 flex flex-col z-50 overflow-hidden">
          {/* Header */}
          <div className="bg-slate-900 p-3 flex items-center gap-2">
            <span className="text-white font-bold text-sm flex-1">💬 Chat Intern</span>
          ...
          {error && (
            <div className="px-3 py-2 bg-rose-50 border-b border-rose-200 text-xs text-rose-600">
              {error}
            </div>
          )}
          ...
          <div className="flex flex-1 min-h-0">
            {/* User list */}
            <div className="w-16 bg-slate-50 border-r border-slate-100 flex flex-col items-center py-2 gap-1 overflow-y-auto">
              {users.map((u) => {
                ...
                return (
                  ...
                  >
                    ...
                    {(u.status === 'active' || u.status === 'activ') && (
                      <span className="absolute -bottom-0.5 -right-0.5 w-2.5 h-2.5 bg-emerald-400 rounded-full border border-white"></span>
                    )}
                  </button>
                );
              })}
            ...
            <div className="flex-1 flex flex-col min-w-0">
              <div className="px-3 py-2 border-b border-slate-100 bg-white">
                <p className="text-xs font-bold text-slate-700">{activeUser?.full_name || activeUser?.name || '—'}</p>
              ...
              <div className="p-2 border-t border-slate-100 flex gap-2">
                <input
                  ...
                  placeholder="Mesaj..."
                  className="flex-1 border border-slate-200 rounded-xl px-3 py-2 text-xs focus:outline-none focus:border-violet-400 transition-colors"
                />
                <button
                  onClick={sendMsg}
                  className="bg-violet-600 text-white p-2 rounded-xl hover:bg-violet-700 transition-colors"
                >
      ...
      <button
        onClick={() => setOpen((o) => !o)}
        className="fixed bottom-5 right-5 w-14 h-14 bg-violet-600 hover:bg-violet-700 text-white rounded-full shadow-xl flex items-center justify-center transition-all hover:scale-105 active:scale-95 z-50"
      >
